...
"""
Ray single-controller trainer for the correct-prefix control (extends VERL ``RayPPOTrainer``).
...
* ``K`` "sub" rollouts: continuation from a noisy assistant prefix. By default
  (``trainer.noise_source = "partial_correct"``), the prefix is drawn from a partial
  correct solution in the ``correct_answer_with_boxed`` column (produced by
...
For sub-rollouts, the correct prefix is folded into the response window after
...
* ``"shift"`` (default): response width = ``R``; trailing ``p_i`` rollout tokens are
  ...
  rollouts); the correct prefix gets ``response_mask = 1`` so PPO loss covers
...
* ``"cutdown"``: response width = ``R``; same length-fair truncation as ``"shift"``,
  but the correct prefix gets ``response_mask = 0`` so PPO loss only covers
...
* ``"none"``: response width = ``R + max_partial_len``; NO truncation — total per-row
  ...
  generated token). The correct prefix gets ``response_mask = 0`` (no gradient on the
  off-policy prefix); all ``R`` generated tokens participate in PPO loss.

In every mode the correct prefix remains visible to the reward manager via
...
from recipe.correct_prefix.dynamic_rho import DynamicRhoController
from recipe.correct_prefix.length_reward import (
    apply_dynamic_length_reward,
    apply_response_clip_penalty,
)
from recipe.correct_prefix.noise_metrics import compute_paired_noise_acc_metrics
from recipe.correct_prefix.per_sample_curriculum import (
    PerSampleCorrectPrefixCurriculum,
    has_usable_correct_solution,
)
from recipe.correct_prefix.prefix_cut import cut_correct_solution_prefix
...
def ensure_problem_ids_on_dataset(dataset) -> None:
    ...
    if "problem_id" not in df.column_names:
        ...
        print(f"[correct prefix] dataset missing 'problem_id'; assigned 1..{n}.")
...
def problem_id_avg_acc_distribution_metrics(problem_id_to_avg_acc: dict) -> dict:
    ...
    out = {
        "correct_prefix/problem_id_avg_acc/n_problems": float(n),
        "correct_prefix/problem_id_avg_acc/n_eq_0": float(n_eq_0),
        "correct_prefix/problem_id_avg_acc/n_eq_1": float(n_eq_1),
        "correct_prefix/problem_id_avg_acc/n_in_open_0_0.5": float(n_open_0_05),
        "correct_prefix/problem_id_avg_acc/n_in_open_0.5_1": float(n_open_05_1),
    }
    if n > 0:
        finite = [float(a) for a in problem_id_to_avg_acc.values() if np.isfinite(a)]
        out["correct_prefix/problem_id_avg_acc/mean"] = float(np.mean(finite))
    return out
...
# Trainer
# -----------------------------------------------------------------------------
class RayCorrectPrefixTrainer(RayPPOTrainer):
    """
    ...
    * ``"partial_correct"`` (default): first ``part_response_ratio`` of a row drawn
      from ``correct_answer_with_boxed``.
    * ``"random_tokens"``: ``random_noise_len`` random tokenizer ids sampled from
      the tokenizer vocabulary.

    After rollout the correct prefix is folded
    ...
    * ``"none"``    — response width = ``R + max_partial_len``, NO truncation
    ...
    def _create_dataloader(
        ...
        """Filter unusable rows before correct-prefix step counts are fixed."""
        ...
        if not self._correct_prefix_enabled() or getattr(self, "_correct_prefix_pool_filtered", False):
            return
        ...
        kept_indices = [
            ...
            if has_usable_correct_solution(item.get("correct_answer_with_boxed", None))
        ]
        ...
        if len(kept_indices) < train_batch_size:
            raise ValueError(
                "Correct-prefix control pool has fewer usable rows than train_batch_size after "
                "removing samples without correct_answer_with_boxed: "
                f"usable={len(kept_indices)}, train_batch_size={train_batch_size}."
            )

        self.train_dataset.dataframe = dataframe.select(kept_indices)
        self._correct_prefix_pool_filtered = True
        self._correct_prefix_pool_original_size = original_size
        self._correct_prefix_pool_removed_count = removed_count
        print(
            "[correct prefix] removed rows without usable correct_answer_with_boxed: "
    ...
    def _correct_prefix_enabled(self) -> bool:
        raw = self.config.trainer.get("correct_prefix_curriculum_enabled", False)
        if isinstance(raw, str):
            return raw.strip().lower() in ("1", "true", "yes", "y", "on")
        return bool(raw)

    def _init_correct_prefix_curriculum(self) -> PerSampleCorrectPrefixCurriculum:
        controller = getattr(self, "_correct_prefix_curriculum", None)
        ...
        if n_main != 0 or k_noise != 16:
            raise ValueError(
                "Correct-prefix control requires exactly 16 noise rollouts per problem: "
                "actor_rollout_ref.rollout.n=0 and trainer.sub_rollout_k=16."
            )
        if str(self.config.trainer.get("noise_source", "partial_correct")).lower() != "partial_correct":
            raise ValueError("Correct-prefix control requires trainer.noise_source='partial_correct'.")
        if bool(self.config.trainer.get("use_dapo", False)):
            raise ValueError(
                "trainer.use_dapo is incompatible with the correct-prefix active batch."
            )
        ...
        if gen_batch_size != train_batch_size:
            raise ValueError(
                "Correct-prefix control requires data.gen_batch_size == data.train_batch_size."
            )

        trainer_cfg = self.config.trainer
        controller = PerSampleCorrectPrefixCurriculum(
            problem_ids=list(self.train_dataset.dataframe["problem_id"]),
            batch_size=train_batch_size,
            initial_rho=trainer_cfg.get("correct_prefix_initial_rho", 0.5),
            min_rho=trainer_cfg.get("correct_prefix_min_rho", 0.0),
            max_rho=trainer_cfg.get("correct_prefix_max_rho", 0.5),
            target_accuracy=trainer_cfg.get("correct_prefix_target_accuracy", 0.75),
            alpha=trainer_cfg.get("correct_prefix_alpha", 0.2),
            history_window=trainer_cfg.get("correct_prefix_history_window", 10),
            min_history=trainer_cfg.get("correct_prefix_min_history", 2),
            slope_threshold=trainer_cfg.get("correct_prefix_slope_threshold", 0.0075),
        )
        self._correct_prefix_curriculum = controller
        return controller

    def _correct_prefix_state_path(self) -> Path:
        return (
            Path(self.config.trainer.default_local_dir)
            / f"global_step_{self.global_steps}"
            / "correct_prefix_curriculum.json"
        )

    def _load_correct_prefix_state(self) -> None:
        if not self._correct_prefix_enabled() or self.global_steps <= 0:
            return
        state_path = self._correct_prefix_state_path()
        if not state_path.exists():
            raise FileNotFoundError(
                f"Missing Correct-prefix control state for resumed step {self.global_steps}: "
                f"{state_path}"
            )
        self._init_correct_prefix_curriculum().load_state_dict(
            json.loads(state_path.read_text(encoding="utf-8"))
        )
        print(f"[correct prefix] restored active pool state from {state_path}.")

    def _save_correct_prefix_state(self) -> None:
        if not self._correct_prefix_enabled():
            return
        state_path = self._correct_prefix_state_path()
        state_path.parent.mkdir(parents=True, exist_ok=True)
        state_path.write_text(
            json.dumps(self._init_correct_prefix_curriculum().state_dict(), indent=2, sort_keys=True),
            encoding="utf-8",
        )

    def _iter_correct_prefix_epoch_batches(self):
        """Repeatedly collate the current active indices instead of walking the pool."""
        from verl.utils.dataset.rl_dataset import collate_fn

        curriculum = self._init_correct_prefix_curriculum()
    ...
    def fit(self):
        ...
        # Build problem_id -> raw item mapping (used to fetch correct_answer_with_boxed and prompt
        ...
        correct_prefix_curriculum = None
        if self._correct_prefix_enabled():
            correct_prefix_curriculum = self._init_correct_prefix_curriculum()

        self._load_checkpoint()
        if correct_prefix_curriculum is not None:
            self._load_correct_prefix_state()
            ...
            pprint(
                "[correct prefix] enabled: "
                f"original_pool_size={self._correct_prefix_pool_original_size}, "
                f"removed_without_noise={self._correct_prefix_pool_removed_count}, "
                f"pool_size={correct_prefix_curriculum.pool_size}, "
                f"active_batch_size={correct_prefix_curriculum.batch_size}, "
                "noise_rollouts_per_problem=16, "
                f"initial_rho={correct_prefix_curriculum.initial_rho}, "
                f"rho_range=[{correct_prefix_curriculum.min_rho}, {correct_prefix_curriculum.max_rho}], "
                f"target_accuracy={correct_prefix_curriculum.target_accuracy}, "
                f"alpha={correct_prefix_curriculum.alpha}, "
                f"history_window={correct_prefix_curriculum.history_window}, "
                f"min_history={correct_prefix_curriculum.min_history}, "
                f"slope_abs_threshold={correct_prefix_curriculum.slope_threshold}"
            )
        ...
        if self._uses_dynamic_rho():
            ...
            pprint(
                "[correct prefix] dynamic rho enabled: "
        ...
        for epoch in range(self.config.trainer.total_epochs):
            epoch_batches = (
                self._iter_correct_prefix_epoch_batches()
                if correct_prefix_curriculum is not None
            ...
            for batch_dict in epoch_batches:
                ...
                if correct_prefix_curriculum is not None:
                    ...
                    metrics.update(correct_prefix_curriculum.update(problem_id_to_avg_acc))
                if (
                ...
                if self.config.trainer.save_freq > 0 and (
                    ...
                    with marked_timer("save_checkpoint", timing_raw, "green"):
                        ...
                        self._save_correct_prefix_state()
    ...
    def _select_correct_solutions(self, problem_id, k: int) -> List[str]:
        """Pick up to ``k`` correct solutions for ``problem_id`` (cycling/replacement when needed)."""
        item = self.all_train_items.get(problem_id)
        if item is None:
            return []
        corrects = item.get("correct_answer_with_boxed", None)
        if corrects is None:
            return []
        # ``correct_answer_with_boxed`` may come back as a numpy/Arrow sequence; coerce to list[str].
        try:
            corrects = [corrects] if isinstance(corrects, str) else list(corrects)
        except TypeError:
            return []
        corrects = [w for w in corrects if isinstance(w, str) and w.strip()]

        corrects = corrects[:1]
        if not corrects or k <= 0:
            return []
        # Use a deterministic ordering plus cycle/sample when there are fewer than k candidates.
        if len(corrects) >= k:
            return corrects[:k]
        # Cycle through the available corrects to reach exactly k items.
        return [corrects[j % len(corrects)] for j in range(k)]
    ...
    def _build_token_prefixed_inputs(
        ...
        if not prefix_token_ids:
            raise ValueError("Empty correct prefix tokens.")
        ...
        if prefix_token_len <= 0:
            raise ValueError("No correct prefix tokens.")
        ...
        # If post-padding truncation dropped some of the correct prefix tokens, adjust
        ...
        if prefix_token_len <= 0:
            raise ValueError("No correct prefix tokens remain after prompt truncation.")
    ...
    def _build_text_prefixed_inputs(
        ...
        prompt, so the model continues writing directly from that correct prefix.
        """
        if not isinstance(prefix_text, str) or prefix_text == "":
            raise ValueError("Empty correct prefix text.")
        ...
        if prefix_token_len <= 0:
            raise ValueError("No correct prefix tokens after tokenization.")
    ...
    def _build_partial_inputs(
        ...
        """
        Render a prompt where the assistant message is a prefix of ``correct_text``.
        ``trainer.partial_correct_cut_strategy`` controls whether the requested token
        ...
        """
        cut_strategy = str(
            self.config.trainer.get("partial_correct_cut_strategy", "token")
        ).lower()
        prefix_cut = cut_correct_solution_prefix(
            tokenizer=self.tokenizer,
            text=correct_text,
    ...
    def _fold_partial_into_response(
        ...
        ``partial_correct_len + kept_continuation_len <= R`` independent of ``p_i``;
        ...
        * ``mask_partial_correct = False`` (``"shift"``): partial-wrong gets ``mask = 1``;
          PPO loss covers ``[partial_correct, kept_continuation]``. More signal but
          potentially less stable when the prefix is heavily off-policy.
        * ``mask_partial_correct = True``  (``"cutdown"``): partial-wrong gets ``mask = 0``;
        ...
        * drops the trailing ``p_i`` tokens of the rollout — these are the last ``p_i``
        ...
        contains exactly ``[partial_correct, kept_continuation]`` (and only the generated
        ...
        * ``new_prompts[i]`` has the same ``P`` columns, but with ``p_i`` extra leading
          PADs (the real prompt content has shrunk by ``p_i`` because partial_correct
          moved into the response). For main rollouts (``p_i = 0``) prompt content is
          preserved exactly.
        * ``new_responses[i] = [partial_correct (p_i), generated[:R - p_i]]`` — exactly
        ...
        * ``response_mask[i]`` is ``1`` (shift) or ``0`` (cutdown) over the leading
        ...
        # real generated tokens past the R-budget once partial_correct is accounted for).
    ...
    def _fold_partial_extended(
        ...
        trailing ``p_i`` tokens to enforce ``partial_correct + kept <= R``.
        ...
        * ``new_responses[i]`` has width ``R + max_partial_len`` and equals
          ``[partial_correct (p_i), generated (R), trailing PADs (max_partial_len - p_i)]``.
        ...
        * ``response_mask[i]`` is ``0`` over the leading ``p_i`` partial-wrong
        ...
        ``[partial_correct, full_continuation]`` for sub-rollouts.
    ...
    def _rollout_and_compute_reward(self, batch_dict, _metrics, timing_raw):
        ...
        noise_source = str(self.config.trainer.get("noise_source", "partial_correct")).lower()
        if noise_source not in ("partial_correct", "random_tokens"):
            raise ValueError(
                "trainer.noise_source must be 'partial_correct' or 'random_tokens', "
                f"got {noise_source!r}."
            )
        partial_correct_cut_strategy = str(
            self.config.trainer.get("partial_correct_cut_strategy", "token")
        ).lower()
        if partial_correct_cut_strategy not in ("token", "line"):
            raise ValueError(
                "trainer.partial_correct_cut_strategy must be 'token' or 'line', "
                f"got {partial_correct_cut_strategy!r}."
            )
        ...
        if k > 0 and noise_source == "partial_correct" and not self._correct_prefix_enabled():
        ...
        if k > 0 and noise_source == "partial_correct" and problem_ids is None:
            raise ValueError(
                "noise_source='partial_correct' with sub_rollout_k > 0 requires "
                "'problem_id' in the non-tensor batch "
                "so that correct_answer_with_boxed can be fetched from self.all_train_items."
            )
        ...
        for b in range(B):
            ...
            corrects = []
            if noise_source == "partial_correct":
                corrects = self._select_correct_solutions(problem_id, k)
            if noise_source == "partial_correct" and not corrects:
                problems_without_corrects += 1
            problem_rho = None
            if self._correct_prefix_enabled():
                problem_rho = self._init_correct_prefix_curriculum().rho_for_problem(problem_id)

            for j in range(k):
                slot_rho = 0.0
                try:
                    if noise_source == "partial_correct":
                        # The controller samples rho once per problem. All 16
                        ...
                        # rho=0 is the fully unassisted endpoint. The reverse
                        # curriculum normally starts from max rho and shortens the
                        # correct prefix as accuracy rises above the target.
                        ...
                        correct_text = corrects[j] if j < len(corrects) else None
                        if not correct_text:
                            raise ValueError("No correct solution available for this sub-rollout slot.")
                        (
                            ids,
                            mask,
                            pos,
                        ...
                        if partial_correct_cut_strategy == "line":
                    ...
                    print(
                        f"[correct prefix] failed to build {noise_source} prompt for problem_id={problem_id!r}: "
        ...
        # Fold the correct prefix into the response window. Three modes:
        ...
        if max_partial_len > 0:
            if partial_mode == "none":
                ...
                _metrics["correct_prefix/sub_rollout/dropped_tokens_sum"] = float(
                    dropped_tokens_sum
                )
                _metrics["correct_prefix/sub_rollout/n_rows_with_drops"] = float(
                    n_rows_with_drops
                )
                if n_sub_rows_with_prefix > 0:
                    _metrics[
                        "correct_prefix/sub_rollout/dropped_tokens_mean_per_prefix_row"
                ...
                new_batch = self._fold_partial_into_response(
                    ...
                    partial_lens_np,
                    max_partial_len,
                    mask_partial_correct=(partial_mode == "cutdown"),
                )

        _metrics["correct_prefix/sub_rollout/n_main_rows"] = float(B * n)
        _metrics["correct_prefix/sub_rollout/n_sub_rows_planned"] = float(B * k)
        _metrics["correct_prefix/sub_rollout/n_sub_rows_with_prefix"] = float(n_sub_rows_with_prefix)
        _metrics["correct_prefix/sub_rollout/n_sub_rows_zero_noise"] = float(n_sub_rows_zero_noise)
        _metrics["correct_prefix/sub_rollout/n_sub_rows_fallback"] = float(n_sub_rows_fallback)
        _metrics["correct_prefix/sub_rollout/n_problems_without_corrects"] = float(problems_without_corrects)
        _metrics["correct_prefix/sub_rollout/max_partial_len"] = float(max_partial_len)
        _metrics["correct_prefix/sub_rollout/noise_source_partial_correct"] = float(
            noise_source == "partial_correct"
        )
        _metrics["correct_prefix/sub_rollout/noise_source_random_tokens"] = float(
            noise_source == "random_tokens"
        )
        if noise_source == "partial_correct" and part_response_ratio_samples:
            ratio_arr = np.asarray(part_response_ratio_samples, dtype=np.float32)
            _metrics["correct_prefix/sub_rollout/part_response_ratio_mean"] = float(
                np.mean(ratio_arr)
            )
            _metrics["correct_prefix/sub_rollout/part_response_ratio_min"] = float(
                np.min(ratio_arr)
            )
            _metrics["correct_prefix/sub_rollout/part_response_ratio_max"] = float(
                np.max(ratio_arr)
            )
            if realized_part_response_ratio_samples:
                ...
                _metrics[
                    "correct_prefix/sub_rollout/realized_part_response_ratio_mean"
                ] = float(np.mean(realized_ratio_arr))
                _metrics[
                    "correct_prefix/sub_rollout/realized_part_response_ratio_min"
                ] = float(np.min(realized_ratio_arr))
                _metrics[
                    "correct_prefix/sub_rollout/realized_part_response_ratio_max"
                ] = float(np.max(realized_ratio_arr))
            _metrics["correct_prefix/sub_rollout/n_line_boundary_prefixes"] = float(
                n_line_boundary_prefixes
            )
            _metrics["correct_prefix/sub_rollout/n_line_boundary_fallbacks"] = float(
                n_line_boundary_fallbacks
            )
            if self._uses_dynamic_rho():
                _metrics.update(self._get_dynamic_rho_controller().metrics())
            if self._correct_prefix_enabled():
                _metrics.update(self._init_correct_prefix_curriculum().metrics())
                _metrics["correct_prefix/curriculum/pool_original_size"] = float(
                    self._correct_prefix_pool_original_size
                )
                _metrics["correct_prefix/curriculum/pool_removed_without_noise"] = float(
                    self._correct_prefix_pool_removed_count
                )
        if noise_source == "random_tokens":
            _metrics["correct_prefix/sub_rollout/random_noise_len_tokens"] = float(random_noise_len)
            _metrics["correct_prefix/sub_rollout/random_noise_vocab_size"] = float(
                random_noise_vocab_size
            )
            _metrics["correct_prefix/sub_rollout/random_noise_exclude_special"] = float(
                random_noise_exclude_special
            )
        if n_sub_rows_with_prefix > 0:
            _metrics["correct_prefix/sub_rollout/mean_partial_len"] = (
                float(sum_partial_lens) / float(n_sub_rows_with_prefix)
            )
            _metrics["correct_prefix/sub_rollout/max_observed_partial_len"] = float(max_observed_partial)
        ...
        # slots that fell back to a no-prefix rollout (because no correct solution
        ...
        if k > 0 and sub_rollout_separate_loss_group:
            ...
            _metrics["correct_prefix/sub_rollout/loss_group_split"] = 1.0
            _metrics["correct_prefix/sub_rollout/loss_multiplier_sub"] = float(
                sub_loss_multiplier
            )
            _metrics["correct_prefix/sub_rollout/n_rows_loss_group_sub"] = float(
                int(is_sub.sum())
            )
            _metrics["correct_prefix/sub_rollout/n_rows_loss_group_main"] = float(
                int((~is_sub).sum())
            )
        else:
            _metrics["correct_prefix/sub_rollout/loss_group_split"] = 0.0
        ...
        with marked_timer("reward", timing_raw, "yellow"):
            ...
            _metrics["correct_prefix/response_clip_reward/enabled"] = float(
            ...
            if response_clip_reward_penalty != 0.0:
                ...
                _metrics["correct_prefix/response_clip_reward/penalty"] = (
                    response_clip_reward_penalty
                )
                _metrics["correct_prefix/response_clip_reward/n_clipped"] = float(
                    n_clipped
                )
                _metrics["correct_prefix/response_clip_reward/clip_ratio"] = float(
            ...
            _metrics["correct_prefix/length_reward/enabled"] = float(length_reward_enabled)
            if length_reward_enabled:
                ...
                _metrics["correct_prefix/length_reward/max_response_length"] = float(
                    generated_response_width
                )
                _metrics["correct_prefix/length_reward/dynamic_cache_mean"] = float(
                    prefix_lengths_f.mean().item()
                )
                _metrics["correct_prefix/length_reward/penalty_start_length_mean"] = float(
                    penalty_start_lengths.mean().item()
                )
                _metrics["correct_prefix/length_reward/min_factor"] = min_factor
                _metrics["correct_prefix/length_reward/scope_is_all"] = float(
                    length_reward_scope == "all"
                )
                _metrics["correct_prefix/length_reward/n_correct"] = float(
                    int(correct_mask.sum().item())
                )
                _metrics["correct_prefix/length_reward/n_penalized"] = float(
                    int((effective_penalties > 0).sum().item())
                )
                _metrics["correct_prefix/length_reward/penalty_mean"] = float(
                    effective_penalties.mean().item()
                )
                _metrics["correct_prefix/length_reward/generated_length_mean"] = float(
                ...
                if torch.any(correct_mask):
                    ...
                    _metrics[
                        "correct_prefix/length_reward/correct_generated_length_mean"
                    ] = float(correct_lengths.mean().item())
                    _metrics[
                        "correct_prefix/length_reward/correct_dynamic_cache_mean"
                    ] = float(prefix_lengths_f[correct_mask].mean().item())
                    _metrics["correct_prefix/length_reward/correct_factor_mean"] = float(
                        correct_factors.mean().item()
                    )
                    _metrics["correct_prefix/length_reward/correct_factor_min"] = float(
                        correct_factors.min().item()
                    )
                    _metrics[
                        "correct_prefix/length_reward/correct_reward_before_mean"
                    ] = float(reward_before_length[correct_mask].mean().item())
                    _metrics[
                        "correct_prefix/length_reward/correct_reward_after_mean"
    ...
    def _dapo_filter_kept_problems(self, batch: DataProto):
        ...
        filter_metrics = {
            "correct_prefix/dapo/n_problems_total": float(len(order_pids)),
            "correct_prefix/dapo/n_problems_kept": float(n_kept),
            "correct_prefix/dapo/n_problems_dropped": float(len(order_pids) - n_kept),
            "correct_prefix/dapo/n_rows_total": float(n_rows),
            "correct_prefix/dapo/n_rows_kept": float(len(kept_idxs)),
        }
    ...
    def _relabel_uids_and_split_groups(self, batch: DataProto, metrics: dict) -> DataProto:
        ...
        if k_sub > 0 and sub_rollout_separate_adv_uid:
            ...
            metrics["correct_prefix/sub_rollout/adv_uid_split"] = 1.0
            metrics["correct_prefix/sub_rollout/n_rows_adv_uid_sub"] = float(
                int(is_sub_row.sum())
            )
            metrics["correct_prefix/sub_rollout/n_rows_adv_uid_main"] = float(
                int((~is_sub_row).sum())
            )
        else:
            metrics["correct_prefix/sub_rollout/adv_uid_split"] = 0.0
    ...
    def _dapo_step(
        ...
        with marked_timer("step", timing_raw):
            ...
            metrics["correct_prefix/dapo/num_gen_batches"] = float(num_gen)
            metrics["correct_prefix/dapo/num_kept_problems_cumulative"] = float(num_kept)
            ...
            metrics["correct_prefix/dapo/n_rows_used_in_update"] = float(traj_bsz)
            metrics["correct_prefix/dapo/n_problems_used_in_update"] = float(prompt_bsz)
