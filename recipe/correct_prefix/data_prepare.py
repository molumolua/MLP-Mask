...
5. 额外把原 parquet 复制一份，并新增一列 ``correct_answer_with_boxed``
   （``list[str]``，存放该样本所有「含 boxed 且答对」的 rollout 全文；若没有
   则为空列表），落到 ``<output-dir>/<原文件名>.with_correct_boxed.parquet``。
6. 控制台 + ``summary.json`` 报告：
       - 模型的总体正确率（micro，所有 rollout 上的平均）；
       - 每个样本至少有一个 rollout 答错的概率；
       - 至少有一个「含 boxed 且答对」rollout 的样本数。
...
from recipe.correct_prefix.verifier import extract_last_boxed  # noqa: E402
...
def main() -> None:
    ...
    # 每个样本对应一个 list[str]，存所有「含 boxed 且答对」的 rollout 全文。
    # 长度恒等于 n_samples，与原 df 一一对应；没有就给空列表。
    per_sample_correct_with_boxed: List[List[str]] = []
    ...
    for i, output in enumerate(tqdm(outputs, desc="Scoring")):
        ...
        accs: List[float] = []
        boxed_hits = 0
        correct_with_boxed_texts: List[str] = []
        ...
        for j, gen in enumerate(sample_outputs):
            ...
            # 收集「含 boxed 且被 verifier 判定正确」的 rollout 全文。
            if boxed is not None and acc >= 1.0:
                correct_with_boxed_texts.append(response_text)
        ...
        per_sample_correct_with_boxed.append(correct_with_boxed_texts)
    ...
    # 在原 parquet 上新增 `correct_answer_with_boxed` 列后另存
    df_out = df.copy()
    df_out["correct_answer_with_boxed"] = pd.Series(
        per_sample_correct_with_boxed, index=df_out.index, dtype=object
    )
    augmented_parquet = args.output_dir / (args.dataset.stem + ".with_correct_boxed.parquet")
    ...
    n_with_correct_boxed = int(sum(1 for xs in per_sample_correct_with_boxed if xs))
    total_correct_boxed = int(sum(len(xs) for xs in per_sample_correct_with_boxed))
    ...
    summary = {
        ...
        "num_samples_with_correct_boxed": n_with_correct_boxed,
        "total_correct_boxed_rollouts": total_correct_boxed,
    ...
    print(
        f"有 boxed 且答对的样本    : {n_with_correct_boxed} / {n_samples}"
        f"   ({n_with_correct_boxed / n_samples:.4f})"
    )
    print(f"有 boxed 且答对的 rollout: {total_correct_boxed}")
