...
# Correct-prefix control invariants: one active prompt batch, zero clean/main slots, and
# exactly 16 fixed-rho correct-prefix continuations per prompt.
n_resp_per_prompt=0
sub_rollout_k=16
effective_rollout_n=16

# Every prompt owns an independent rho. Start from a long correct prefix;
# success shortens it, failure lengthens it, and replacements inherit batch mean rho.
correct_prefix_initial_rho=${correct_prefix_initial_rho:-0.5}
correct_prefix_min_rho=${correct_prefix_min_rho:-0.0}
correct_prefix_max_rho=${correct_prefix_max_rho:-0.5}
correct_prefix_target_accuracy=${correct_prefix_target_accuracy:-0.75}
correct_prefix_alpha=${correct_prefix_alpha:-0.2}
correct_prefix_history_window=${correct_prefix_history_window:-5}
correct_prefix_min_history=${correct_prefix_min_history:-2}
# A sample is stable only inside the two-sided band:
# abs(recent_rho_slope) <= correct_prefix_slope_threshold.
# Reverse update: rho <- clip(rho - alpha * (accuracy - target), min, max).
correct_prefix_slope_threshold=${correct_prefix_slope_threshold:-0.02}
...
# GRPO schedule.
epoch=${epoch:-10000}
project_name=${project_name:-CorrectPrefix-Control-4B}
lr=${lr:-1e-6}
...
# correct_answer_with_boxed, treats the remaining rows as one pool, and explicitly
...
TRAIN_FILE=${TRAIN_FILE:-./data/MATH7500.with_correct_boxed.qwen2.5-1.5b.parquet}
...
run_tag="rho${correct_prefix_initial_rho}-${correct_prefix_max_rho}_target${correct_prefix_target_accuracy}_alpha${correct_prefix_alpha}_window${correct_prefix_history_window}_slope${correct_prefix_slope_threshold}_${length_reward_tag}${response_clip_reward_tag}"
experiment_name=${experiment_name:-"none-grpo-correct-prefix-${model_name}-bsz${train_prompt_bsz}-k16-${run_tag}"}
...
PYTHONUNBUFFERED=1 python3 -m recipe.correct_prefix.main_dapo \
    ...
    +trainer.noise_source=partial_correct \
    +trainer.partial_correct_cut_strategy=token \
    ...
    +trainer.correct_prefix_curriculum_enabled=True \
    +trainer.correct_prefix_initial_rho=${correct_prefix_initial_rho} \
    +trainer.correct_prefix_min_rho=${correct_prefix_min_rho} \
    +trainer.correct_prefix_max_rho=${correct_prefix_max_rho} \
    +trainer.correct_prefix_target_accuracy=${correct_prefix_target_accuracy} \
    +trainer.correct_prefix_alpha=${correct_prefix_alpha} \
    +trainer.correct_prefix_history_window=${correct_prefix_history_window} \
    +trainer.correct_prefix_min_history=${correct_prefix_min_history} \
    +trainer.correct_prefix_slope_threshold=${correct_prefix_slope_threshold} \
