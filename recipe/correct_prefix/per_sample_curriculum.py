"""Ordered-pool, per-sample reverse curriculum for the correct-prefix control."""
...
def has_usable_correct_solution(correct_solutions) -> bool:
    """Return whether a pool row can produce a non-empty correct prefix."""
    if correct_solutions is None:
        return False
    if isinstance(correct_solutions, str):
        candidates = [correct_solutions]
    else:
        try:
            candidates = list(correct_solutions)
        except TypeError:
            return False
    return any(
...
@dataclass
class SampleCorrectPrefixState:
    rho: float
...
class PerSampleCorrectPrefixCurriculum:
    """Maintain one fixed correct-prefix ratio per problem in a deterministic active batch."""
    ...
    def __init__(
        ...
        *,
        ...
        if not self.problem_ids:
            raise ValueError("Correct-prefix control requires a non-empty training pool.")
        try:
        ...
        if len(unique_problem_ids) != len(self.problem_ids):
            raise ValueError("Correct-prefix control requires a unique problem_id per pool row.")
        ...
        self.states = {
            index: SampleCorrectPrefixState(rho=self.initial_rho)
    ...
    def rho_for_problem(self, problem_id) -> float:
        ...
        if index is None or index not in self.states:
            raise KeyError(
                f"problem_id {normalized!r} is not in the active correct-prefix batch."
            )
    ...
    def _slope(self, state: SampleCorrectPrefixState) -> float | None:
    ...
    def update(self, problem_id_to_average_accuracy: Mapping) -> dict[str, float]:
        ...
        for index in self.active_indices:
            ...
            state.num_samples += 1
            # Reverse of denoise_v2: strong accuracy removes correct-prefix help,
            # while weak accuracy restores a longer correct prefix.
            state.rho = min(
                self.max_rho,
                max(
                    self.min_rho,
        ...
        if round_restart:
            ...
            self.active_indices = list(range(self.batch_size))
            self.states = {
                index: SampleCorrectPrefixState(rho=replacement_rho)
                for index in self.active_indices
            }
            self.next_pool_index = self.batch_size
            ...
            for index in self.active_indices:
                ...
                self.states[replacement_index] = SampleCorrectPrefixState(rho=replacement_rho)
        ...
        metrics = {
            "correct_prefix/curriculum/active_batch_size": float(len(self.active_indices)),
            "correct_prefix/curriculum/pool_size": float(self.pool_size),
            "correct_prefix/curriculum/pool_cursor": float(self.next_pool_index),
            "correct_prefix/curriculum/covered_this_round": float(self.next_pool_index),
            "correct_prefix/curriculum/coverage_fraction_this_round": float(
                self.next_pool_index / self.pool_size
            ),
            "correct_prefix/curriculum/pool_remaining": float(self.pool_size - self.next_pool_index),
            "correct_prefix/curriculum/pool_round": float(self.pool_round),
            "correct_prefix/curriculum/rounds_completed": float(self.rounds_completed),
            "correct_prefix/curriculum/round_restart_this_step": float(round_restart),
            "correct_prefix/curriculum/samples_introduced_total": float(
                self.samples_introduced_total
            ),
            "correct_prefix/curriculum/accuracy_mean": float(np.mean(average_accuracies)),
            "correct_prefix/curriculum/rho_used_mean": float(np.mean(used_rhos)),
            "correct_prefix/curriculum/rho_replacement": replacement_rho,
            "correct_prefix/curriculum/rho_active_mean": float(np.mean(active_rhos)),
            "correct_prefix/curriculum/rho_active_min": float(np.min(active_rhos)),
            "correct_prefix/curriculum/rho_active_max": float(np.max(active_rhos)),
            "correct_prefix/curriculum/stable_candidates": float(len(stable_indices)),
            "correct_prefix/curriculum/slope_abs_threshold": self.slope_threshold,
            "correct_prefix/curriculum/replaced_this_step": float(replace_count),
            "correct_prefix/curriculum/stable_replaced_this_step": float(stable_replace_count),
            "correct_prefix/curriculum/retired_total": float(self.retired_total),
            "correct_prefix/curriculum/unfilled_retirements": float(unfilled_retirements),
            "correct_prefix/curriculum/pool_cycles_enabled": 1.0,
            "correct_prefix/curriculum/slopes_defined": float(len(finite_slopes)),
        }
        if finite_slopes:
            absolute_slopes = [abs(value) for value in finite_slopes]
            metrics.update(
                {
                    "correct_prefix/curriculum/slope_mean": float(np.mean(finite_slopes)),
                    "correct_prefix/curriculum/slope_min": float(np.min(finite_slopes)),
                    "correct_prefix/curriculum/slope_max": float(np.max(finite_slopes)),
                    "correct_prefix/curriculum/slope_abs_mean": float(np.mean(absolute_slopes)),
                    "correct_prefix/curriculum/slope_abs_min": float(np.min(absolute_slopes)),
                    "correct_prefix/curriculum/slope_abs_max": float(np.max(absolute_slopes)),
                }
            )
        return metrics
    ...
    def metrics(self) -> dict[str, float]:
        active_rhos = [self.states[index].rho for index in self.active_indices]
        return {
            "correct_prefix/curriculum/enabled": 1.0,
            "correct_prefix/curriculum/active_batch_size": float(len(self.active_indices)),
            "correct_prefix/curriculum/pool_size": float(self.pool_size),
            "correct_prefix/curriculum/pool_cursor": float(self.next_pool_index),
            "correct_prefix/curriculum/pool_remaining": float(self.pool_size - self.next_pool_index),
    ...
    def load_state_dict(self, saved: Mapping) -> None:
        if int(saved.get("version", -1)) != self.STATE_VERSION:
            raise ValueError("Unsupported Correct-prefix control curriculum state version.")
        ...
        states: dict[int, SampleCorrectPrefixState] = {}
        for index in active_indices:
            ...
            states[index] = SampleCorrectPrefixState(
                rho=rho,
                history=history,
