...
class PrefixCut:
    """Result of cutting a correct solution into a correct prefix."""
...
def cut_correct_solution_prefix(
    tokenizer: _Tokenizer,
    text: str,
    ratio: float,
    strategy: str = "token",
    ...
    if not isinstance(text, str) or not text:
        raise ValueError("Correct solution must be a non-empty string.")
    ...
    if strategy not in ("token", "line"):
        raise ValueError(
            "partial correct cut strategy must be 'token' or 'line', "
            f"got {strategy!r}."
        )

    token_ids = list(tokenizer.encode(text, add_special_tokens=False))
    if not token_ids:
        raise ValueError("Empty correct solution after tokenization.")
