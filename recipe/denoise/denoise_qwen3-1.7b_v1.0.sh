...
n_resp_per_prompt=${n_resp_per_prompt:-15}
sub_rollout_k=${sub_rollout_k:-1}
...
exp_name=${exp_name:-"${version}-${noise_exp_tag}-adv-split-${sub_rollout_separate_adv_uid}-loss-split-${sub_rollout_separate_loss_group}-sub-mult-${sub_rollout_loss_multiplier}-problem-id-${use_problem_id_as_uid}-response-same-${use_same_uid}-model-${model_name}-lr-${lr}-bsz-${train_prompt_bsz}-n_resp-${n_resp_per_prompt}-mini-${train_prompt_mini_bsz}"}
