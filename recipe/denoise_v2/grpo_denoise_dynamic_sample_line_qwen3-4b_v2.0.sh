...
# DenoiseRL v2: 16 noisy continuations, no clean/main rollout slots.
n_resp_per_prompt=0
sub_rollout_k=16
effective_rollout_n=16

# DAPO dynamic sampling only:
#   * sample_prompt_bsz prompts are generated in each round;
#   * all-correct and all-wrong prompt groups are removed by mean verifier acc;
#   * backward_prompt_bsz retained prompts are used for one optimizer update;
#   * generation repeats until enough prompts survive, with a hard 10-round cap.
sample_prompt_bsz=${sample_prompt_bsz:-48}
backward_prompt_bsz=${backward_prompt_bsz:-16}
backward_prompt_mini_bsz=${backward_prompt_mini_bsz:-${backward_prompt_bsz}}
max_dynamic_sample_rounds=10

# Every active prompt owns an independent rho.
...
run_tag="dynamic-sample-s${sample_prompt_bsz}-b${backward_prompt_bsz}-max${max_dynamic_sample_rounds}-line-none-nolen-rho${v2_initial_rho}-${v2_max_rho}"
experiment_name=${experiment_name:-"grpo-denoise-v2-${model_name}-${run_tag}"}
...
PYTHONUNBUFFERED=1 python3 -m recipe.denoise_v2.main_dapo \
    ...
    data.gen_batch_size=${sample_prompt_bsz} \
    data.train_batch_size=${backward_prompt_bsz} \
    ...
    actor_rollout_ref.actor.ppo_mini_batch_size=${backward_prompt_mini_bsz} \
    ...
    +trainer.dapo_max_num_gen_batches=${max_dynamic_sample_rounds} \
    ...
    +trainer.correct_length_reward_enabled=False \
    +trainer.correct_length_reward_min_factor=0.0 \
    +trainer.length_reward_scope=all \
    +trainer.response_clip_reward_penalty=0.0 \
