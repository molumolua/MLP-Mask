"""Low-overhead sampled gradient diagnostics for antithetic cross-route KL."""
...
class SampledGradientTracker:
    ...
    def finish_update(self) -> dict[str, float]:
        ...
        return {
            "mlp_antithetic/cross_kl/main_grad_rms_sampled": main_rms,
            "mlp_antithetic/cross_kl/aux_grad_rms_sampled": auxiliary_rms,
            "mlp_antithetic/cross_kl/aux_to_main_grad_ratio_sampled": ratio,
            "mlp_antithetic/cross_kl/main_aux_grad_cosine_sampled": cosine,
            "mlp_antithetic/cross_kl/main_aux_grad_angle_degrees_sampled": (
                math.degrees(math.acos(cosine)) if cosine_denom > 0.0 else 0.0
            ),
            "mlp_antithetic/cross_kl/grad_angle_defined": float(cosine_denom > 0.0),
            "mlp_antithetic/cross_kl/grad_ratio_defined": float(main_rms > 0.0),
            "mlp_antithetic/cross_kl/main_grad_l2_sampled": math.sqrt(main_sq),
            "mlp_antithetic/cross_kl/aux_grad_l2_sampled": math.sqrt(auxiliary_sq),
            "mlp_antithetic/cross_kl/gradient_sample_fraction": sample_count / total_numel,
            "mlp_antithetic/cross_kl/gradient_sample_count": sample_count,
        }
