"""Detached-teacher forward KL with full or top-k-plus-tail distributions.

Only the student receives KL gradients. Temporary vocabulary calculations are
chunked by response token, including the exact logsumexp tail derivative.
"""
...
def build_distribution(logits: torch.Tensor, *, top_k: int = 64,
    ...
    """Cache the source route's detached probabilities and its top-k partition."""
...
def slice_distribution_rows(distribution: Distribution, row_token_counts: tuple[int, ...],
                            start: int, end: int) -> Distribution:
    """Slice examples from a flattened response distribution, including empty rows."""
    if not 0 <= start < end <= len(row_token_counts):
        raise IndexError("invalid teacher row slice")
    if any(count < 0 for count in row_token_counts) or sum(row_token_counts) != distribution.log_probs.shape[0]:
        raise ValueError("teacher row counts do not match the cached response tokens")
    token_start = sum(row_token_counts[:start])
    token_end = token_start + sum(row_token_counts[start:end])
    return Distribution(
        distribution.log_probs[token_start:token_end],
        None if distribution.token_ids is None else distribution.token_ids[token_start:token_end],
    )
...
class _ForwardKL(torch.autograd.Function):
    ...
    def forward(ctx, logits, target_log_probs, chunk_size, token_ids):
        if logits.ndim != 2:
            raise ValueError("forward KL requires a two-dimensional token-by-vocabulary tensor")
        ...
        if target_log_probs.shape != expected_shape:
            raise ValueError("forward KL requires matching token counts and categorical distributions")
        if target_log_probs.requires_grad:
            raise ValueError("forward KL requires a detached teacher target")
        if chunk_size <= 0:
            raise ValueError("chunk_size must be positive")
        dtype = torch.float64 if logits.dtype == torch.float64 else torch.float32
        ...
        for start in range(0, logits.shape[0], chunk_size):
            ...
            value += (q * (logq - logp)).sum()
            # d KL(detached teacher q || student p) / d categorical logits(p).
            dcategories = p * q.sum(-1, keepdim=True) - q
            if token_ids is None:
...
def forward_kl_sum(logits: torch.Tensor, target_log_probs: torch.Tensor,
                   *, chunk_size: int = 128, token_ids: torch.Tensor | None = None) -> torch.Tensor:
    """Sum KL(detached teacher || student), differentiating student logits only.
    ...
    """
    return _ForwardKL.apply(logits, target_log_probs, chunk_size, token_ids)
