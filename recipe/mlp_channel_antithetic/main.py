"""Entry point for antithetic MLP-channel GRPO."""
...
def main(config):
    runner = ray.remote(num_cpus=1)(MLPChannelAntitheticTaskRunner)
    run_ppo(config, task_runner_class=runner)


class MLPChannelAntitheticTaskRunner:
    def __init__(self):
        self.role_worker_mapping = {}
        self.mapping = {}

    def run(self, config):
        from recipe.mlp_channel_antithetic.worker import (
            MLPChannelAntitheticActorRolloutRefWorker,
        )
        ...
        print(
            f"MLPChannelAntitheticTaskRunner hostname: {socket.gethostname()}, "
            f"PID: {os.getpid()}"
        )
        ...
        trainer = MLPChannelAntitheticTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
