...
# MLP block without replacement. Every clean backward collects current-step RMS
# activation; beta=0 uses the previous step as the next comparison baseline.
# Relative-activation percentile rank sets a weight in [1, weighted_max_ratio]; there
...
export experiment_name=${experiment_name:-"grpo-${model_name}-clean8-masked8-relativeactivation-weightedrandom-every-step${mask_ratio}-r${weighted_max_ratio}-p${weighted_rank_power}-seed${random_seed}"}
