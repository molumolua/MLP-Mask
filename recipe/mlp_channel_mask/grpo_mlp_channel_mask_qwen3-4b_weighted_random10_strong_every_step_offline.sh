...
# Stronger per-layer weighted-random intervention at the same 1% mask budget.
...
export experiment_name=${experiment_name:-"grpo-${model_name}-clean8-masked8-weightedrandom-strong-every-step${mask_ratio}-r${weighted_max_ratio}-p${weighted_rank_power}-seed${random_seed}"}
