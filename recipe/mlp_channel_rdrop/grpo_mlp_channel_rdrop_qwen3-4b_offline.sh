...
parameter_update_diagnostics_enabled=${parameter_update_diagnostics_enabled:-True}

# Per prompt: half mask A, half mask B; preserve the common GRPO uid.
...
if (( n_rollouts < 2 || n_rollouts % 2 != 0 )); then
    echo "n_rollouts must be an even integer >= 2" >&2
    exit 2
fi

"${python_bin}" -m recipe.mlp_channel_rdrop.main \
    ...
    actor_rollout_ref.mlp_channel_rdrop.enabled=True \
    actor_rollout_ref.mlp_channel_rdrop.auxiliary_enabled=${auxiliary_enabled} \
    actor_rollout_ref.mlp_channel_rdrop.mask_ratio=${mask_ratio} \
    actor_rollout_ref.mlp_channel_rdrop.random_seed=${random_seed} \
    actor_rollout_ref.mlp_channel_rdrop.kl_coef=${kl_coef} \
    actor_rollout_ref.mlp_channel_rdrop.kl_top_k=${kl_top_k} \
    actor_rollout_ref.mlp_channel_rdrop.kl_token_chunk_size=${kl_token_chunk_size} \
    actor_rollout_ref.mlp_channel_rdrop.micro_batch_size_per_gpu=${kl_micro_batch_size_per_gpu} \
    actor_rollout_ref.mlp_channel_rdrop.gradient_sample_size_per_rank=${gradient_sample_size_per_rank} \
    actor_rollout_ref.mlp_channel_rdrop.parameter_update_diagnostics_enabled=${parameter_update_diagnostics_enabled} \
