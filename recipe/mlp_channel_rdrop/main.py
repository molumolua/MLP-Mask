"""Entry point for rdrop MLP-channel GRPO."""
...
from .trainer import MLPChannelRDropTrainer
from .validation import validate_recipe_config


@hydra.main(config_path="config", config_name="ppo_mlp_channel_rdrop", version_base=None)
def main(config):
    runner = ray.remote(num_cpus=1)(MLPChannelRDropTaskRunner)
    run_ppo(config, task_runner_class=runner)


class MLPChannelRDropTaskRunner:
    def __init__(self):
        self.role_worker_mapping = {}
        self.mapping = {}

    def run(self, config):
        from recipe.mlp_channel_rdrop.worker import (
            MLPChannelRDropActorRolloutRefWorker,
        )
        ...
        print(
            f"MLPChannelRDropTaskRunner hostname: {socket.gethostname()}, "
            f"PID: {os.getpid()}"
        )
        ...
        if config.algorithm.use_kl_in_reward or config.actor_rollout_ref.actor.use_kl_loss:
            self.role_worker_mapping[Role.RefPolicy] = ray.remote(
                MLPChannelRDropActorRolloutRefWorker
            )
        ...
        trainer = MLPChannelRDropTrainer(
            config=config,
            tokenizer=tokenizer,
            processor=processor,
