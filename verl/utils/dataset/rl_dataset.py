...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        # Online prompt rewrite: append this string to the last user message at __getitem__ time.
        # Empty string disables the rewrite. Applied before chat-template / tokenization, so both
        # `input_ids` and `raw_prompt_ids` automatically reflect the modified prompt.
    ...
    def _build_messages(self, example: dict):
        ...
        # Online suffix: append to the LAST user message before any further transformation
        # (chat template / multimodal segmentation). Cheaper and safer than re-tokenizing
        # downstream, and keeps `input_ids` / `raw_prompt_ids` consistent.
        if self.user_prompt_suffix:
            messages = self._append_user_prompt_suffix(messages, self.user_prompt_suffix)
    ...
    @staticmethod
    def _append_user_prompt_suffix(messages: list, suffix: str) -> list:
        """Return a shallow-copied messages list whose last user message has `suffix` appended.

        Handles both plain-string content and pre-segmented multimodal content (list of
        {"type": ..., "text"|"image"|...} dicts) so it stays correct when called before or
        after the multimodal split. Original list/dicts are not mutated to avoid leaking
        the suffix back into the underlying HF dataset row.
        """
        ...
        for i in range(len(new_messages) - 1, -1, -1):
            ...
            if isinstance(content, str):
                ...
                new_msg["content"] = list(content) + [{"type": "text", "text": suffix}]
            else:
                new_msg["content"] = f"{content}{suffix}"
